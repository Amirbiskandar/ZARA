	...
	switch(menu)
		{
			...
			case 5:
				...
				setcursor(0,3);//(x,y)
				printf("[ ] Yes\n[ ] No");
				i = selector(2,3);//(number of options,y)
				system("cls");
				fflush(stdout);
				}
...
void box(int y, int offset){
		setcursor(1,y+offset-2);
		printf(" ");
		setcursor(1,y+offset-1);
		printf("%c",254);
		setcursor(1,y+offset);
		printf(" ");
		setcursor(1,y+offset-1);
}
...
void introduction(){
	...
	trex = selector(2,3);//(number of options,y-1)
	int i;
	if(trex==1||trex==2)
	for(i=0;i<time/2;i++){
